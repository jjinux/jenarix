...
#define JX_HEAP_TRACKER_SPINLOCK
  ...
  JX_INLINE jx_status jx__os__process_init(int argc, char *argv[],
    ...
    if((num_size != sizeof(jx_int)) ||
       ...
       (ptr_size != sizeof(void *)) || (ob_size != sizeof(jx_ob))) {
      jx_fprintf(stderr, "FatalError: size mismatch: program vs. library\n");
      jx_fprintf(stderr, "FatalError: numbers:         %2d    vs.   %2d\n",
                 num_size, (int) sizeof(jx_int));
      jx_fprintf(stderr, "FatalError: tiny strings:    %2d    vs.   %2d\n",
                 tiny_size, (int) JX_TINY_STR_SIZE);
      jx_fprintf(stderr, "FatalError: pointers:        %2d    vs.   %2d\n",
                 ptr_size, (int) sizeof(void *));
      jx_fprintf(stderr, "FatalError: objects:         %2d    vs.   %2d\n",
...
#ifdef JX_HEAP_TRACKER_MUTEX
        if(JX_OK(status))
          status = jx_os_mutex_init(&jx_os_Process->heap_mutex);
        /*      if(JX_OK(status)) status = jx_os_mutex_unlock(&jx_os_Process->heap_mutex); */
#endif
        if(JX_OK(status))
          status = jx_os_tls_init(&jx_os_Process->node_tls);
        return status;
      }
    } else {
      return JX_SUCCESS;
...
#ifdef JX_HEAP_TRACKER_MUTEX
    if(JX_OK(status))
      status = jx_os_mutex_destroy(&jx_os_Process->heap_mutex);
#endif
    if(JX_OK(status))
      status = jx_os_tls_destroy(&jx_os_Process->node_tls);
    if(jx_os_Process)
      jx_os_free(jx_os_Process);
    return status;
  }
