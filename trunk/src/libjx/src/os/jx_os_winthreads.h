...
extern "C" {
  ...
  JX_INLINE_STATIC jx_os_atomic32 jx_os_atomic32_exchange(jx_os_atomic32 * atomic,
...
/* Eliminated some memory leaks, gsw@agere.com */
  ...
  JX_INLINE_STATIC BOOL LeaveNonRecursiveMutex(PNRMUTEX mutex) {
    ...
    mutex->thread_id = 0;
    {
      ...
      return (owned < 0) || SetEvent(mutex->hevent);    /* Other threads are waiting, wake one on them up */
    }
  }
...
#define noJX_OS_TLS_TRACE_DEBUG
  ...
  JX_INLINE_STATIC jx_status jx__os_thread_start(jx_os_thread * thread,
                                                 void (*func) (void *), void *arg) {
    jx_status status = JX_PTR(thread);
    if(JX_OK(status)) {
      thread->thread =
        CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE) func, arg, 0, &thread->thread_id);
      if(!thread->thread) {
  ...
  JX_INLINE_STATIC jx_status jx__os_cond_init(jx_os_cond * cond) {
    ...
    if(JX_OK(status)) {
      ...
      InitializeCriticalSectionAndSpinCount(&cond->critical_section, 0x80000400);
  ...
  JX_INLINE_STATIC jx_status jx__os_cond_wait(jx_os_cond * cond, jx_os_mutex * mutex) {
    ...
    if(JX_OK(status)) {
      ...
      if(last_waiter && broadcasting) {
        ...
        SignalObjectAndWait(cond->broadcast_complete, mutex->mutex.hevent, INFINITE,
                            FALSE);
  ...
  JX_INLINE_STATIC jx_status jx__os_cond_timedwait(jx_os_cond * cond, jx_os_mutex * mutex,
                                                   jx_size usec) {
    jx_status status = JX_PTR(cond && mutex);
    if(JX_OK(status)) {
      int broadcasting;
  ...
  JX_INLINE_STATIC jx_status jx__os_rlock_acquire(jx_os_rlock * rlock,
                                                  jx_boolean blocking) {
    jx_status status = JX_PTR(rlock);
    if(JX_OK(status)) {
